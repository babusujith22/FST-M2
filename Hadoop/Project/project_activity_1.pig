-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/Sujith/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/Sujith/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/Sujith/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/Sujith/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/Sujith/outputs' USING PigStorage('\t');
